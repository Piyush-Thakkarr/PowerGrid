...
async def get_comparison(user_id: UUID) -> dict:
    row = await fetchrow(
        """WITH ref AS (
           ...
           user_profile AS (
               SELECT state, household_size FROM user_profiles WHERE user_id = $1
           ),
           state_avg AS (
           ...
           user_rank AS (
               SELECT rank FROM (
                   SELECT user_id, RANK() OVER (ORDER BY SUM(energy_kwh) ASC) AS rank
                   FROM consumption_data, ref
                   WHERE timestamp >= ref.latest_month - interval '1 month'
                     AND timestamp < ref.latest_month
                   GROUP BY user_id
           ...
           SELECT COALESCE(user_monthly.kwh, 0) AS user_kwh,
                  ...
                  user_profile.state, user_profile.household_size
    ...
    return {
        ...
        "yourRank": row["user_rank"], "totalUsers": row["total_users"],
        "percentile": percentile, "state": row["state"],
